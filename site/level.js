function Wall(x, y, w, h, level_num) {
    ...
    this.h = h;
    this.color = (function () {
    const pal = (CFG && CFG.colors && CFG.colors.wallsPalette) ? CFG.colors.wallsPalette
      : ["#1919A6", "#E75480", "#228B22", "#B22222", "#d3d3d3", "#fffdd0", "#FFD700"];
    return pal[Math.floor(level.level_num / 2)] || pal[pal.length - 1];
    })();
...
function Pellet(x, y, w, h) {
    ...
    this.d = this.w / 5;
    this.r = this.d / 2;
    this.color = (CFG && CFG.colors && CFG.colors.pellet) ? CFG.colors.pellet : "#ffb8ae";
        ...
        for (var y = 0; y < blueprint.length; y++) {
            for (var x = 0; x < blueprint[y].length; x++) {
                ...
                if (cell == "W") {
                    ...
                    const speeds = (CFG && CFG.gameplay && CFG.gameplay.ghosts && CFG.gameplay.ghosts.speedByLevel)
                        ? CFG.gameplay.ghosts.speedByLevel : [1.5, 2, 2, 2.5, 2.5, 2.5, 3];
                    const base = (CFG && CFG.gameplay && CFG.gameplay.ghosts && CFG.gameplay.ghosts.scaredBaseMs) ? CFG.gameplay.ghosts.scaredBaseMs : 5500;
                    const dec  = (CFG && CFG.gameplay && CFG.gameplay.ghosts && CFG.gameplay.ghosts.scaredDecrementPerLevel) ? CFG.gameplay.ghosts.scaredDecrementPerLevel : 500;
                    const min  = (CFG && CFG.gameplay && CFG.gameplay.ghosts && CFG.gameplay.ghosts.scaredMinMs) ? CFG.gameplay.ghosts.scaredMinMs : 500;

                    var speed = speeds[level.level_num - 1] || speeds[speeds.length - 1];
                    var scared_time = Math.max(base - (this.level_num * dec), min);
                    ...
                    ghosts.push(new Ghost(
                        ...
                        level.level_num, speed, scared_time
                    ));
