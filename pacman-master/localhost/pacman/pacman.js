function Pacman(player, x, y, w, h) {

    this.player = player;

  // NECESARIO: usado por lógica de colisiones/movimiento
  this.getCurrentCell = function () {
    return [Math.floor(this.x / 30), Math.floor(this.y / 30)];
  };

  // colores configurables
  this.color = (function () {
    if (!CFG || !CFG.colors) return (player === 1 ? "yellow" : "red");
    return (player === 1 ? (CFG.colors.pacman1 || "yellow") : (CFG.colors.pacman2 || "red"));
  })();

  this.d = w;
  this.r = this.d / 2;
  this.mouth_bottom = 9;
  this.mouth_top = -9;
  this.dir = [0, -1];
  this.next_dir = [0, -1];
  this.x = x;
  this.y = y;

  // gameplay configurables
  this.speed = (CFG && CFG.gameplay && CFG.gameplay.pacman && CFG.gameplay.pacman.speed) ? CFG.gameplay.pacman.speed : 3;
  this.mouth_speed = 5;
  this.facing_angle = 90;
  this.next_facing_angle = 90;
  this.mouth_dir = 1; // (-1) cierra / (1) abre
  this.move_count = 30 / this.speed;
  this.current_cell = this.getCurrentCell();
  this.clear_ahead = false;
  this.pellets_eaten = 0;
  this.score = 0;
  this.lives = (CFG && CFG.gameplay && CFG.gameplay.pacman && CFG.gameplay.pacman.lives) ? CFG.gameplay.pacman.lives : 3;
  this.is_dying = false;
  this.is_moving = false;




    this.moveMouth = function () {
        this.mouth_bottom += (this.mouth_speed * this.mouth_dir);
        this.mouth_top -= (this.mouth_speed * this.mouth_dir);

        if (this.mouth_bottom > 50 || this.mouth_bottom < 5) {
            this.mouth_dir *= -1;
        }

    }




    this.hits = function (arr) {

    for (var i = arr.length - 1; i > -1; i--) {
    var d = dist(this.x, this.y, arr[i].x, arr[i].y);

    if (d < this.r + arr[i].r) {

      // --- pellet normal ---
      if (arr[i].type === "pellet" && !arr[i].empty) {
        arr[i].empty = true;
        this.pellets_eaten++;
        this.score++;

        // sonido al comer CADA pellet
        pacman_chomp_sound.play();

        // vida extra cada N pellets (desde config.json; por defecto 400)
        var extraEvery = (CFG && CFG.gameplay && CFG.gameplay.pacman && CFG.gameplay.pacman.extraLifeEvery)
          ? CFG.gameplay.pacman.extraLifeEvery
          : 400;

        if (extraEvery > 0 && this.pellets_eaten % extraEvery === 0) {
          boom_sound.play();
          this.lives++;
        }
      }

      // --- power pellet ---
      if (arr[i].type === "power_pellet" && !arr[i].empty) {
        arr[i].empty = true;
        this.score += 50;
        power_pellet_sound.play();
        this.d += 10;

        ghosts.forEach(function (ghost) {
          ghost.getScared();
        });
      }

      // --- choque con fantasma ---
      if (arr[i].type === "ghost" && !arr[i].is_dying && !this.is_dying) {
        if (arr[i].is_scared) {
          arr[i].die(); // muere el fantasma
          this.score += 100;
        } else {
          this.die(); // muere pacman
        }
      }
    }
  }
}


    this.die = function () {

        this.lives--;
        this.is_dying = true;
        this.is_moving = false;
        pacman_death_sound.play();

        var pacman = this;
        setTimeout(function () {
            pacman.restart();

            ghosts.forEach(function (ghost) {
                ghost.restart();
            });

        }, 2000);


    }


    this.restart = function () {

        if (this.lives > 0) {
            this.d = 30;
            this.x = x;
            this.y = y;
            this.dir = [0, -1];
            this.next_dir = [0, -1];
            this.facing_angle = 90;
            this.next_facing_angle = 90;
            this.is_dying = false;
        }

    }



    this.move = function () {

        if ((this.x - 15) % 30 === 0 && (this.y - 15) % 30 === 0) { //in the middle of a cell block, look


            if (this.lookAround(this.next_dir)) {
                this.changeDir();
            }
            this.clear_ahead = this.lookAround(this.dir);

        }


        if (this.clear_ahead) {
            this.x += this.dir[0] * this.speed; // * 1.3;
            this.y += this.dir[1] * this.speed;
            this.moveMouth();


            //boundries
            if (this.x > width - this.r) {
                this.x = this.d;
            }
            if (this.x < this.r) {
                this.x = width - this.d;
            }

        }


    }


    this.lookAround = function (dir) {

        var cc = this.current_cell = this.getCurrentCell();

        //if wanting to change directions, look that way

        var next_dir_cell = [cc[0] + dir[0], cc[1] + dir[1]];

        //if it's clear to turn then turn
        var next_dir_cell_content = level.current_map[next_dir_cell[1]].slice(next_dir_cell[0], next_dir_cell[0] + 1); //map symbol

        return next_dir_cell_content != "W"; // not a wall, coast is clear, change directions or keep moving


    }




    //change direction now
    this.changeDir = function () {

        this.dir = this.next_dir;
        this.facing_angle = this.next_facing_angle;
    }


    //set a direction to be turned soon
    this.changeNextDir = function (dir, next_facing_angle) {

        this.next_dir = dir;
        this.next_facing_angle = next_facing_angle;

        //if this is a 180 turn, turn immediately
        if (Math.max(this.facing_angle, this.next_facing_angle) - Math.min(this.facing_angle, this.next_facing_angle) === 180) {
            this.changeDir();
        }

    }



    // pacman.js — dentro de Pacman(...)
// DINO PIXELADO estilo Chrome pero más chico, cola larga y bracito T-Rex
this.show = function () {
  push();
  translate(this.x, this.y);

  // un poco más chico que el Pacman original
  const size = this.d * 0.8;
  const px = size / 16;
  const ox = -8 * px, oy = -8 * px;

  noStroke();
  rectMode(CORNER);

  const block = (x, y, w = 1, h = 1, col = color(139, 69, 19)) => {
    fill(col);
    rect(ox + x * px, oy + y * px, w * px, h * px);
  };

  const C = color(93, 195, 60); // marrón estilo tierra
  const OJO = 0;                // negro

  // --- cabeza ---
  block(9, 0, 3, 1, C);
  block(8, 1, 5, 1, C);
  block(8, 2, 5, 1, C);
  block(8, 3, 5, 1, C);
  block(7, 4, 6, 1, C);
  block(6, 5, 6, 1, C);
  block(5, 6, 6, 1, C);

  // boca
  block(13, 3, 2, 1, C);

  // ojo
  block(10, 1, 1, 1, OJO);

  // --- cuerpo ---
  block(4, 7, 7, 1, C);
  block(3, 8, 7, 1, C);
  block(2, 9, 7, 1, C);
  block(1,10, 7, 1, C);
  block(1,11, 6, 1, C);
  block(0,12, 6, 1, C);

  // --- cola más larga ---
  block(-1, 9, 1, 1, C);
  block(-1,10, 1, 1, C);
  block(-1,11, 1, 1, C);
  block(-2,10, 1, 1, C);

  // --- brazos más largos tipo T-Rex ---
  block(8, 7, 1, 1, C);  // hombro derecho
  block(8, 8, 5, 1, C);  // brazo hacia abajo
  block(7, 7, 1, 1, C);  // hombro izquierdo
  block(7, 8, 1, 4, C);  // brazo hacia abajo

  // --- patas ---
  block(4,13, 2, 3, C);
  block(7,13, 2, 3, C);

  pop();
};


}


